import {
  // twoSum,
  // twoSumBruteForce as twoSum,
  twoSumTwoPassHashes as twoSum
} from './two-sum';

describe('two sum test case', () => {
  it('empty array', () => {;
    expect(twoSum([], 9)).toEqual([]);
  });

  it('there are indexes', () => {
    expect(twoSum([7, 2, 11, 15], 9)).toEqual([0, 1]);
    expect(twoSum([2, 1, 7, 15, 11], 13)).toEqual([0, 4]);
    expect(twoSum([3, 2, 4], 6)).toEqual([1, 2]);
    expect(twoSum([2, 1, 4, 4], 8)).toEqual([2, 3]);
    expect(twoSum([2, 7, 13, 5, 4, 13, 5], 10)).toEqual([3, 6]);
  });

  it('there is no such indexes', () => {
    expect(twoSum([1, 2, 3, 9, 10], 8)).toEqual(-1);
    expect(twoSum([2, 7, 11, 15], 8)).toEqual(-1);
  });
});
