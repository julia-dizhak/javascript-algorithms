...
In the problem description, we're told that iff there is a town judge, there'll 
only be one town judge.
It's likely that not all interviewers would tell you directly that there can only 
be one town judge.
If you asked them whether or not there could be more than one town judge, they 
might ask you if there could be.
...
Therefore, fpr a person to maximize their "score", they should be trusted by as 
many people as possible,
and trust as few people as possible.

The maximum indegree in N-1. This represents everybody trusting the person 
(except for themselves, they cannot trust themselves).
...
The town judge is the only person who could possible have indegree - outdegree 
equal to N - 1
...
O(max(N, E) = O(N+E). After all, we don't know whether E or N is the bigger one, 
right?
However, remember how we terminate early if E < N - 1?
Tis means that in the best case, the time complexity is O(1).
And in the worst case, we know that E ≥ N - 1. For the purpose of big-oh notation, 
we ignore the constant of 1.
Therefore, in the worst case, E has to be bigger, and so we can simply drop the N, 
leaving O(E).
...
directed edge going from a to b.
...
directed edges going out of it.
For this graph, the outdegree of the vertex represents the number of other people 
that person trusts.

Likewise, we say that the indegree of a vertex (person) is the number of directed 
edges going into it.
So here, it represents the number of people trusted by that person.

The town judge has an outdegree of 0 and an indegree of N - 1 because they trust 
nobody, and everybody trusts them (except themselves).

Therefore, this problem simplifies to calculating the indegree and outdegree for 
each person
and then checking whether or not any of them meet the criteria of the town judge.
We can calculate the indegrees and outdegrees for everybody, using a single loop 
over the input trust array.
We'll write the results into two arrays.

Complexity analysis
Time complexity is O(E) and the argument about relationship between N and E 
still applies.

Space complexity is O(N)
We allocated 2 arrays; one for the indegrees and the other for the outdegrees.
Each was of length N + 1. Because in big-oh notation we drop constants, this 
leaves us with O(N).
*/
var findJudgeTwoArrays = function(N, trust) {
  ...
  for (let i = 1; i <=N; i++) {
    // The town judge has an outdegree of 0 and an indegree of N - 1 because 
    // they trust nobody, and everybody trusts them (except themselves).
